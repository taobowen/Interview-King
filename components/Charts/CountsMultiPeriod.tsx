...
export default function CountsMultiPeriod({apps}:{apps:ApplicationDoc[]}) {
    ...
    <div className="rounded border bg-white p-4">
      ...
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label"/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="count"/></BarChart>
        </ResponsiveContainer>
      </div>
    </div>
  );
  ...
  return <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
    <Card title="Daily (last 30)" data={byDay}/>
    <Card title="Weekly (last 12)" data={weeks}/>
    <Card title="Monthly (last 12)" data={months}/>
  </div>;
}
